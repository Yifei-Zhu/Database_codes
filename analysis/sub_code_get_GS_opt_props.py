...
class Gaussian16:
   ...
   def get_atom_labels(self):
      ...
      labels = np.expand_dims(atoms, axis=0)
      return labels
...
def writeAllPropsFile(out_file, all_properties):
...
def writeXyzFile(xyz_file,labels,coords):
   with open(xyz_file, 'w') as f1:
      n_atom=len(labels[0])
      f1.write(f'{n_atom}\n\n')
      for i in range(n_atom):
         label = labels[0][i]
         coord = str(coords[i]).lstrip('[').rstrip(']').replace("'",'')
         f1.write(f'{label}    {coord}\n')
      f1.write('\n')

def main_import_write(log_file, out_file, work_type):
   g16_output = Gaussian16(log_file, work_type)
   if work_type =='opt':
      all_properties = {'labels': g16_output.get_atom_labels,
                        'coords': g16_output.get_coords,
   ...
   xyz_file=change_file_extension(log_file, 'xyz')
   writeXyzFile(xyz_file, all_properties['labels'], all_properties['coords'])
   writeAllPropsFile(out_file, all_properties)

def main_import(log_file, work_type):
   g16_output = Gaussian16(log_file, work_type)
   if work_type =='opt':
      all_properties = {'labels': g16_output.get_atom_labels,
                        'coords': g16_output.get_coords,
...
def main_xyz(log_file, xyz_file, work_type):
   g16_output = Gaussian16(log_file, work_type)
   all_properties = {'labels': g16_output.get_atom_labels,
                     'coords': g16_output.get_coords}
   label = all_properties['labels'][0]
   labels =[[chemical_symbols[number] for number in label]]

   coords = all_properties['coords']
   # file_name = os.path.splitext(os.path.basename(log_file))[0]
   writeXyzFile(xyz_file,labels,coords)



if __name__ == '__main__':
   # main()
   # main_import('/data/home/zhuyf/dataset_work/database/qm9_database/Gaussian/37432/37432.log','./aaa.dat','opt+freq')
   main_xyz('/data/home/zhuyf/dataset_work/database/GDB11/split_gdb10/samples_1w_for_calculation/calculation_/gdb11_Gaussian_/584837/584837.log','/data/home/zhuyf/dataset_work/database/GDB11/split_gdb10/samples_1w_for_calculation/calculation_/gdb11_Gaussian_/584837/584837.xyz','opt+freq')
